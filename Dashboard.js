...
export default function Dashboard() {
  ...
  function doExport() {
    ...
    if (tab === 'specialty' || tab === 'products') {
      // Mirror exactly what PivotTable renders on screen: same shift/user/search
      // filtering, same rowKey × user_name cross-tab, summed by valueKey.
      const rowKey = tab === 'specialty' ? 'specialty' : 'product';
      const valueKey = 'call_count';
      const sourceRows = tab === 'specialty' ? filteredSpecialty : filteredProducts;

      const pivotFiltered = sourceRows.filter(r => {
      ...
      const users = [...new Set(pivotFiltered.map(r => r.user_name))].sort();
      const rowKeys = [...new Set(pivotFiltered.map(r => r[rowKey]))].sort();
      const cells = {};
      pivotFiltered.forEach(r => {
        const k = r[rowKey]; if (!cells[k]) cells[k] = {};
        cells[k][r.user_name] = (cells[k][r.user_name] || 0) + (r[valueKey] || 0);
      });

      const label = tab === 'specialty' ? 'Specialty' : 'Product';
      const sh = [[label, ...users, 'Total']];
      rowKeys.forEach(k => {
        let rowTotal = 0;
        ...
        sh.push([k, ...row, rowTotal]);
      });
      const colTotalsRow = ['Total', ...users.map(u =>
        rowKeys.reduce((s, k) => s + (cells[k]?.[u] || 0), 0)
      )];
      colTotalsRow.push(colTotalsRow.slice(1).reduce((s, v) => s + v, 0));
      sh.push(colTotalsRow);

      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(sh), label);
